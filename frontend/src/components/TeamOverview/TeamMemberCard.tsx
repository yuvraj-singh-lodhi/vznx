...
const TeamMemberCard: React.FC<TeamMemberCardProps> = ({ member }) => {
  const workloadPercentage = Math.min(100, (member.assignedTasks / member.capacity) * 100);
  ...
  return (
    <div className="p-4 border border-slate-200 rounded-lg bg-slate-50 transition-shadow hover:shadow-inner">
      <h4 className="text-lg font-semibold text-slate-800 mb-2">{member.name}</h4>
      <div className="flex flex-col">
        <span className="text-xs text-slate-500 mb-1">{member.assignedTasks} Tasks ({workloadPercentage.toFixed(0)}% Capacity)</span>
