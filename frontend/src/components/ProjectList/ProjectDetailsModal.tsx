...
const ProjectDetailsModal: React.FC<ProjectDetailsModalProps> = ({
  ...
  // 🔥 Local state for manual progress input (prevents double update)
  const [localManualValue, setLocalManualValue] = useState(
    manualProgressValue !== undefined ? manualProgressValue : ''
  );

  // When parent updates the value, sync input too
  useEffect(() => {
    if (manualProgressValue !== undefined) {
      setLocalManualValue(manualProgressValue);
    }
  }, [manualProgressValue]);
  ...
  // 🔥 Trigger update ONLY on Enter
  const handleManualKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
      handleManualProgressUpdate(project.id, String(localManualValue));
    }
  };
      ...
      <div className="bg-slate-50 p-4 rounded-lg border border-slate-200">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col items-center shrink-0 relative">
            <label htmlFor="manual-progress" className="text-xs text-slate-500 mb-0.5">
              Manual %
            </label>
            ...
            <input
              ...
              onChange={(e) => setLocalManualValue(e.target.value)}
              onKeyDown={handleManualKeyDown}   // ✔️ Only Enter triggers update
